...
void insertIntoHash(const char* TYPE,
                    zmsg_t *msg,
                    zhash_t *frames) {
//printf("CURRENT MESSAGE SIZE before pop: %d\n", zmsg_size(msg));
...
//---------------------------------------
    char buf[BUFSIZE];
    if(strcmp(TYPE, OPNUM) !=0 || strcmp(TYPE, PAYLOAD)!=0){
            _zframe_str(frame, buf) ;
               printf("TYPE:%s, recv: %s\n", TYPE, buf);
    }
    else if(strcmp(TYPE, PAYLOAD)){
        printf("PAYLOAD FRAME--------%d", zframe_size(frame));
        for(int i=0; i<200; i++)
            printf("%d*********************\n", i);
    }
//----------------------------------------
...
void insertIntoHashAndList(const char* TYPE,
    ...
    if(names!=NULL) zlist_append(names,(void*) TYPE);
}
