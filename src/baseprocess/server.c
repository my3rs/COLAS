...
//    int count = 0;
    while (true) {
        if(DEBUG_MODE)  printf("\twaiting for message\n");
...
void receive_message_frames_at_server_SODAW(
    ...
    if( strcmp(phase_name, READ_DISPERSE) ==0 ) {
        insertIntoHashAndList(META_SEND_TAG, msg, frames, names);
        insertIntoHashAndList(META_READERID, msg, frames, names);
        insertIntoHashAndList(META_READ_TAG, msg, frames, names);
        insertIntoHashAndList(OPNUM, msg, frames, names);
    }
...
/*
    for(int i=0;i<200; i++)
        printf("after push %d frames into hashtable\n", zhash_size(frames));
...
zhash_t *receive_message_frames_at_server(zmsg_t *msg, zlist_t *names) {
    ...
    insertIntoHashAndList(CLIENTID, msg, frames, names);
